...
public class ChatServer extends JFrame implements ActionListener {
	...
	public ChatServer() {
		...
		svname = JOptionPane.showInputDialog("이름을 입력하세요.");
		
		setTitle(svname + " 채팅 중"); // 프레임 타이틀
	...
	private class Receiver extends JTextArea implements Runnable {
		...
		public void run() {
			...
			while (true) {
				...
				this.append("\n" + clname + " < " + msg); // 받은 문자열을 JTextArea에 출력
	...
	public void actionPerformed(ActionEvent e) { // JTextField에 <Enter> 키 처리
		if (e.getSource() == sender) {
			...
			try {
				out.write(svname + ">" + msg + "\n"); // 문자열 전송
				out.flush();
				
				receiver.append("\n" + svname + " > " + msg);// JTextArea에 출력
				int pos = receiver.getText().length();
				receiver.setCaretPosition(pos); // caret 포지션을 가장 마지막으로 이동
